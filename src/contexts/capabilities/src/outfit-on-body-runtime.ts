import { shouldAttemptOnBodyGeneration, type Outfit } from "../../wardrobe/src/index.js";
...
export function createOutfitOnBodyGenerationAttempt(input: {
  ...
  return async function attemptOutfitOnBodyGeneration(outfit: Outfit): Promise<void> {
...
function resolvePhotoOnBodyOutfit(context: any, body: Record<string, unknown>, outfitId: string, outfitImageUrl: string): Outfit | undefined {
  ...
  const stored = Array.isArray(shellConfig.outfits)
    ? shellConfig.outfits.find((outfit: Outfit) => outfit.id === outfitId)
    : undefined;
...
function clearPhotoOnBodyAttempt(context: any, outfit: Outfit): void {
...
function hasPhotoOnBodyImage(context: any, outfit: Pick<Outfit, "id" | "onBodyImageUrl">): boolean {
  const current = context.dailyShellStore.getConfig(context.time.now().date, context.time.timeZone).outfits.find((entry: { id?: string }) => entry.id === outfit.id);
