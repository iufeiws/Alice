...
test("sqlite migration backfills text message event logs into core-facing messages", () => {
  const { store } = createBackfillStore("backfill-message");
...
test("sqlite migration backfills read and reaction event logs into core-facing messages", () => {
  const { store } = createBackfillStore("backfill-events");
  const message = store.listMessagesForConversation("session-backfill", 10)[0];
...
test("sqlite migration backfills message schema version", () => {
  const { db } = createBackfillStore("backfill-version");
  assert.equal(db.prepare("PRAGMA user_version").get().user_version, 8);
});

function createBackfillStore(name: string) {
