...
The chat prompt profile is stored in `core/prompt/prompt-profile.json` and is editable from the admin Prompt page under `Chat`.
...
The talk prompt profile is stored in `core/prompt/talk-prompt-profile.json` and is editable from the admin Prompt page under `Talk`. It was initialized as a copy of the chat prompt profile so Talk starts with the same persona, prompt layers, append layers, and visible-tool switches, then can diverge without changing Chat.

`core/prompt/prompt-api-profile.json` binds Talk to its own `talkPresetName`. If unset, Talk has no explicit API preset binding; setting it does not change Chat or Memorize bindings.

## Memorize

Memorize remains a separate memory-induction path rather than an AgentCore chat/talk loop. Its prompts live in `core/prompt/memorize-prompts.json`, and its API binding remains `memorizePresetName`.
