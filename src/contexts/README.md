...
- `llm-gateway/src/llm-requests.ts`
  - 统一 LLM request sender、tool schema rendering、retry/backoff 和 LLM lifecycle logging。
- `agent-loop/src/application/chat-loop-session-context.ts`
  - 将 chat fixed-prefix append、finish_and_wait resume、`Chat` poll cursor、token estimate 和 session-context helpers 从 `run-chat-loop.ts` 移出。
...
3. [done] 引入 `activeMainLLMSession` 命名和主 session 状态 port；`messagingTools` 默认 `Chat` action=poll scope 已从该 port 获取 session boundary，不再依赖工具内部计数猜测。
...
- `messagingTools` 的默认 `Chat` action=poll scope 必须由主 loop/session 状态明确决定，不能再由工具私有计数跨 agent 边界推断。
