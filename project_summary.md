...
| subagent | `SubAgent` | 持久化 SubAgent 会话（spawn/messages/result/send/status/cancel/fork，走 PiWorker；wait 仅保留在执行层、不暴露给 LLM；公开会话标识使用 nickname，messages 保留 access 语义并返回 Pi 原始 message） |
...
| finish-and-wait | `Yield` | 等待、清空上下文或结束：new 清除当前 LLM 对话并在同一 loop 开启新一轮，同时追加仅供 Core/Albert 使用的 `<Alert info="上下文历史已清空" />`；await 固定等待 15 分钟；schedule（10s–15min 定时返回）实现保留但不在 profile 中暴露；连续 schedule 且无 subagent 运行时拒绝（防空转） |
