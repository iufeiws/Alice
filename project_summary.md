...
| **llm-gateway** | LLM 调用入口。`createOpenAICompatibleClient`（chat/completions + SSE + listModels，assistant response 中括号内容归一化默认关闭）、`llm-requests.ts`（send + buildTools）、`llm-tool-loop.ts`（多轮 function-call loop，round/call 上限 100、continuation 恢复、全局 tool 注册表 `executeRegisteredLLMTool`；模型调用未注册工具时写回同轮失败 tool result 并继续 loop）、运行时钩子（请求日志/usage/subagent transcript，SubAgent 转录落独立库 `memory-files/llm-subagent-sessions.sqlite`，失败降级不中断 LLM）；所有经 Gateway 的请求均携带实际 preset（Base URL）和响应模型作为价格匹配上下文，不按 agent 来源分支。Token Usage 分为三层：usage 事件只记录实际调用事实；`models.dev/api.json` 最新价格表持久化全部 provider、全部 model（含无 API URL 或无完整单价的条目），仅在记录 usage 时发现整表超过一小时才懒更新并原子替换；价格轨迹只在实际调用时按 `(provider, model)` 从最新表匹配并记录，相同价格延长观察区间、变化时新增版本。Base URL 匹配忽略末尾 `/v1`；同一 URL+model 对应多个 provider 时 warning 并按上游顺序取首项；汇总与单条事件按调用时间关联已经观察到的价格版本计算成本。preset 配置（chat/talk 分离；每个 LLM API preset 可独立选择是否沿用进程代理，默认直连）；OpenAI upstream requester 的同一两次尝试路径覆盖建连与调用方响应消费，SSE 读取阶段因内部 timeout/传输错误失败时会在现有 2 秒间隔后重新发起请求，外部 AbortSignal 取消不重试；Pi relay 重建上游请求时透传 Pi 的 `messages`、`tools`、`tool_choice`，模型、stream、采样参数及其他请求参数仍来自 LLM preset；Pi relay 按 preset 的 `stream` 选择上游 JSON/SSE，非流 JSON 转换为 Pi 可消费的 SSE；chat/talk 收到上游 response 时立即写入 usage，目录更新与价格匹配异步执行、失败仅记录 warning 且不丢失 usage；延迟 response transcript 在格式化后以显式关联 ID 提交，不依赖 request 对象身份；`scripts/backfill-token-usage-from-logs.ts` 可从保留的上游 SSE 原始日志按 model 与时间关联历史 usage，默认 dry-run，`--apply` 时以来源键幂等写入 |
