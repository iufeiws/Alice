...
9. [in_progress] `agent-loop-runtime.requestRun(kind)` 已支持并优先执行 `prepareChat/prepareTalk` 返回的 prepared spec；API 生产 chat/talk wiring 只注册 prepared 入口，legacy `runChat/runTalk` 仅作为测试/兼容 fallback。下一步继续将 chat/talk 的 session 构筑细节从 `AgentCore`/talk adapter 上移到 `agent-loop-runtime` 内部。
