...
| `waiting` | 等用户反馈 | 8-15 秒 | 处理用户消息后保持 `waiting`。 | 5 分钟未收到新消息，降级到 `idle`。 |
...
等待用户反馈状态。收到用户消息时按 8-15 秒延迟回复。5 分钟没有新消息时降级到 `idle`。
...
- `/force_wake`：处理后处于 `waiting`，清理睡眠茧指针，并清理活跃 LLM session。
...
- `/force_wake` 是特殊命令：记录入站活动后，直接将状态设为 `waiting`，清理睡眠茧指针，并清理活跃 LLM session，不进入普通消息处理。
...
- `waiting` 的回复延迟为 8-15 秒，5 分钟无消息后降级到 `idle`。
