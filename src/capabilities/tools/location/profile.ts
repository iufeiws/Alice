...
export const panoramaTool: ToolDefinition = {
  ...
  passRenderText: true,
  description: "控制alice的当前位置, selfie时会是当前所在位置的场景。action=current 查看当前所在历史影像的的描述(会拉取影像文件); action=send 将当前 pano 图片发送给<user>(会拉取影像文件)；action=move 按 World Wanderer 当前选路规则移动一次；action=teleport 按经纬度传送到最近 pano、重置轨迹并清除导航目标；action=navigation 将 World Wanderer 的导航目标设为指定经纬度。",
  inputSchema: {
    type: "object",
    properties: {
      action: { type: "string", enum: ["current", "send", "move", "teleport", "navigation"] },
...
export const locationToolText = {
  ...
  moveUnavailable: "location_move_unavailable",
