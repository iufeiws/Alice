...
系统/运行日志位于 `logs/`，包括 `logs/system`、`logs/message/message-logs.sqlite` 和 `logs/talk/talk.sqlite`。conversation-hub聊天消息位于 `memory-files/alice.sqlite`；清理聊天历史时不要修改 `logs/`。运行时代码来自 `dist`，因此代码变更后需要重启 API 进程。
