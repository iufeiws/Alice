...
```text
contexts/conversation-hub/src/adapters/sqlite-conversation-store.ts
```
...
`messages` 是 conversation-hub会话状态表。它为每条入站或出站消息保存一行，包含内容、发送者、时间、发送状态、已读/撤回标记与聚合 reaction。Agent loop 会基于这个表构造上下文。
...
```text
contexts/conversation-hub/src/adapters/file-log-store.ts
```
