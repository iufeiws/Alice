...
## TTS Translation Prompt

The TTS plugin sends the translation instruction as the `system` message and the dynamic text as the `user` message. For caching, only the repeated static `system` prompt is useful. The previous prompt is likely too close to or below the 128-token floor:
...
Local rough estimate for the recommended prompt is about 132 tokens by Alice's simple CJK estimator. DeepSeek's tokenizer may differ, so verify with several real TTS plugin calls and check that `tts` token usage begins showing non-zero cache hits. If it still reports zero, add one more static rule rather than changing the dynamic user text.
