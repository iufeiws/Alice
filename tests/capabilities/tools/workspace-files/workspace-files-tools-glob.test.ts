...
test("Glob returns recursive matches sorted by mtime", async () => {
  const { root, tools } = makeWorkspace("workspace-glob-mtime");
...
test("Glob supports brace patterns", async () => {
  const { root, tools } = makeWorkspace("workspace-glob-brace");
...
test("Glob truncates large result sets", async () => {
  const { root, tools } = makeWorkspace("workspace-glob-many");
...
test("Glob rejects roots outside workspace", async () => {
  const { tools } = makeWorkspace("workspace-glob-outside");
